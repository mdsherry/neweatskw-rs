...
use crate::facility::{get_facilities, Facility};

fn add_to_queue(db: &Connection, facility: Facility) -> Result<(), Error> {
    db.execute(
        "INSERT INTO queue (facilities_id) VALUES (?)",
        &[&facility.id],
    )?;
    Ok(())
}
...
fn get_recent(db: &Connection, days: i32) -> Result<Vec<Facility>, Error> {
    let mut stmt = db.prepare(&format!(
        "SELECT id, name, address, city FROM facilities WHERE creation >= date('now', '-{} days')",
        days
    ))?;
    let facilities = stmt.query_map([], |row| {
        Ok(Facility {
            id: row.get(0)?,
            name: row.get(1)?,
            address: row.get(2)?,
    ...
    Ok(facilities.collect::<Result<Vec<_>, _>>()?)
}
    ...
    if facility.is_restaurant() && facility.is_in_town() {
        ...
        if query.query(&[&facility.id])?.next()?.is_some() {
            ...
            let mut stmt = db
                ...
                .unwrap();
            stmt.execute(&[&date as &dyn ToSql, &facility.id])?;
        } else {
...
pub fn update(args: Update) -> Result<(), Error> {
    ...
    match args.get_recent {
        Some(days) => {
            for result in get_recent(&db, days)? {
                println!("{:?}", result);
                if args.enqueue {
